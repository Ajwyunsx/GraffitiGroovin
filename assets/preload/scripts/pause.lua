...
function onCreate()
	...
	addLuaSprite('bg')
      makeLuaSprite('right4', 'pause/right4', -1000, 210);
	scaleObject('right4', 0.7, 0.7);
        setObjectCamera('right4', 'other')
                addLuaSprite('right4',true)
...
function onUpdate()
	if tinocoPaused == true then -- if paused
		...
		doTweenX('right4', 'right4', 766, 0.2, 'linear') -- right img slide anim
		doTweenAlpha('lefa', 'right4', 1, 0.001, 'linear') -- right alpha
		setPropertyFromClass('Conductor', 'songPosition', getPropertyFromClass('Conductor', 'songPosition')-16) -- pause the song
		...
		doTweenX('right4', 'right4', 1200, tweentime, 'linear') -- adios right img
		doTweenAlpha('right4a', 'right4', 0, 0.1, 'linear')
